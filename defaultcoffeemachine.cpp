...
bool DefaultCoffeeMachine::createDefaulDB(std::string db_name)
{
    ...
    ret = ret && assert(coffeeMachine_DB.insertPerson("Oleksandra", "EmployedAdmin", "Beuthstr. 14, 12163 Berlin", 1, 1, 0) == true, "Insert Person01 failed");
    ret = ret && assert(coffeeMachine_DB.insertPerson("Christian", "EmployedNormalPerson", "Kaiserin-Augusta-Allee 77, 10589 Berlin", 1, 0, 0) == true, "Insert Person02 failed");
    ret = ret && assert(coffeeMachine_DB.insertPerson("Erwin", "EmployedNormalPerson", "Beusselstr. 22, 10567 Berlin", 1, 0, 0) == true, "Insert Person03 failed");
    ret = ret && assert(coffeeMachine_DB.insertPerson("Katharina", "EmployedNormalPerson", "Klarenbachstr. 43, 10270 Berlin", 1, 0, 0) == true, "Insert Person04 failed");
    ret = ret && assert(coffeeMachine_DB.insertPerson("Mathias", "EmployedAdmin", "Quedlinburger Str. 54, 10340 Berlin", 1, 1, 0) == true, "Insert Person05 failed");
    ret = ret && assert(coffeeMachine_DB.insertPerson("Tobias", "EmployedStaff", "Neues Ufer 18, 10490 Berlin", 1, 0, 1) == true, "Insert Person06 failed");
    ret = ret && assert(coffeeMachine_DB.insertPerson("Viktoria", "EmployedStaff", "Mierendorffstr. 7, 14163 Berlin", 1, 0, 1) == true, "Insert Person07  failed");
